...
class MAICAgent(nn.Module):
    ...
    def forward(self, inputs, hidden_state, bs, test_mode=False, **kwargs):
        ...
        latent_parameters = self.embed_net(h)                       # [bs * n_agents, n_agents * latent_dim * 2]
        latent_parameters[:, -self.n_agents * self.latent_dim:] = th.clamp(
            th.exp(latent_parameters[:, -self.n_agents * self.latent_dim:]),
            min=self.args.var_floor)                                # 对多元高斯混合模型的参数进行处理，取指数保证均值非负。

        latent_embed = latent_parameters.reshape(bs * self.n_agents, self.n_agents * self.latent_dim * 2)   # [bs * n_agents, n_agents * latent_dim * 2]
        ...
        latent = latent.reshape(bs * self.n_agents * self.n_agents, self.latent_dim) # [bs * n_agents * n_agents, latent_dim]

        h_repeat = h.view(bs, self.n_agents, -1).repeat(1, self.n_agents, 1).view(bs * self.n_agents * self.n_agents, -1) # [bs * n_agents * n_agents, hidden_dim]
        msg = self.msg_net(th.cat([h_repeat, latent], dim=-1)).view(bs, self.n_agents, self.n_agents, self.n_actions)   # [bs, n_agents, n_agents, n_actions]
        
        query = self.w_query(h).unsqueeze(1)    # [bs * n_agents, 1, attention_dim]
        key = self.w_key(latent).reshape(bs * self.n_agents, self.n_agents, -1).transpose(1, 2) # [bs * n_agents, attention_dim, n_agents]
        alpha = th.bmm(query / (self.args.attention_dim ** (1/2)), key).view(bs, self.n_agents, self.n_agents)  # [bs, n_agents, n_agents]
        for i in range(self.n_agents):
            alpha[:, i, i] = -1e9
        alpha = F.softmax(alpha, dim=-1).reshape(bs, self.n_agents, self.n_agents, 1) # [bs, n_agents, n_agents, 1]

        if test_mode:
            alpha[alpha < (0.25 * 1 / self.n_agents)] = 0

        gated_msg = alpha * msg     # [bs, n_agents, n_agents, n_actions]
    ...
    def calculate_action_mi_loss(self, h, bs, latent_embed, q):
        latent_embed = latent_embed.view(bs * self.n_agents, 2, self.n_agents, self.latent_dim) # [bs * n_agents, 2, n_agents, latent_dim]
        g1 = D.Normal(latent_embed[:, 0, :, :].reshape(-1, self.latent_dim), latent_embed[:, 1, :, :].reshape(-1, self.latent_dim) ** (1/2))
        hi = h.view(bs, self.n_agents, 1, -1).repeat(1, 1, self.n_agents, 1).view(bs * self.n_agents * self.n_agents, -1) # [bs * n_agents * n_agents, hidden_dim]
        ...
        one_hot_a = one_hot_a.view(bs * self.n_agents * self.n_agents, -1)  # [bs * n_agents * n_agents, n_actions]

        latent_infer = self.inference_net(th.cat([hi, one_hot_a], dim=-1)).view(bs * self.n_agents * self.n_agents, -1) # [bs * n_agents * n_agents, latent_dim*2]
