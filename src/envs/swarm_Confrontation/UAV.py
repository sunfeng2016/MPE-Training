...
class AgentState(object):
    def __init__(self):
        # physical position, 2d array
        self.pos = None
        # physical velocity, 1d scalar
        self.vel = None
        # physical heading, 1d scalar
        self.ang = None

    @property
    def dir(self):
        return np.array([np.cos(self.ang), np.sin(self.ang)])
    
class AccAction(object):
    def __init__(self, acc_num=5, max_acc=5.0):
        # number of acceleration actions
        self.num = acc_num
        # The max acceleration
        self.max = max_acc
        # acc_num should be an odd number
        assert self.num % 2 == 1
        # acc action list
        self.actions = np.linspace(-self.max, self.max, self.num)

class HeadingAction(object):
    def __init__(self, heading_num=5, max_heading=1.0):
        # number of heading actions
        self.num = heading_num
        # The max heading
        self.max = max_heading
        # heading_num should be an odd number
        assert self.num % 2 == 1
        # heading action list
        self.actions = np.linspace(-self.max, self.max, self.num)

class AttackAction(object):
    def __init__(self, attack_num=3):
        # number of attack actions
        self.num = attack_num
        # attack action list
        self.actions = ["explode", "collide", "soft_kill"]

class ActionSpace(object):
    def __init__(self, acc_num=5, max_acc=5.0, heading_num=5, max_heading=1.0, attack_num=3):
        
        assert acc_num % 2 == 1
        assert heading_num % 2 == 1
        
        self.acc_action = np.linspace(-max_acc, max_acc, acc_num)
        self.heading_action = np.linspace(-max_heading, max_heading, heading_num)
        self.attack_action = np.array([0, 1, 2])    # ["explode", "collide", "soft_kill"]

        # self.acc_action = AccAction(acc_num=acc_num, max_acc=max_acc)
        # self.heading_action = HeadingAction(heading_num=heading_num, max_heading=max_heading)
        # self.attack_action = AttackAction(attack_num=attack_num)

class Agent(object):
    def __init__(self, iden=0, index=0, defender=False) -> None:
        # Agent properties
        self.iden = iden
        self.index = index
        ...
        self.acctacking_radius = 300.0
        # Angle of sector attacking zone (degrees)
        self.acctacking_angle = 34.0 * np.pi / 180
        ...
        self.soft_kill_time = 3.0

        # max velocity (m/s)
        self.max_vel = 45.0 if self.defender else 40.0
        # min velocity (m/s)
        self.min_vel = 20.0 if self.defender else 15.0

        # out of range time (seconds)
        self.out_of_range_time = 10.0
...
class World(object):
    def __init__(self):
        ...
        # Add agents
        self.red_agents = [Agent(iden=i, index=i, defender=True) for i in range(self.n_reds)]
        self.blue_agents = [Agent(iden=i+self.n_reds, index=i) for i in range(self.n_blues)]
    ...
    def render(self, frame_num=0, save_frames=False):
        ...
        # 渲染红方基地
        red_base_center_world = np.array([2250, 0])
        red_base_radius_world = 1250
        red_base_center_screen = self.transform_position(red_base_center_world)
        red_base_radius_screen = int(red_base_radius_world / self.size_x * self.screen_width)
        pygame.draw.circle(self.screen, (255, 0, 0), red_base_center_screen, red_base_radius_screen, 2)
