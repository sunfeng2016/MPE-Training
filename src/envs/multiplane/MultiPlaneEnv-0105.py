...
class MultiPlaneEnv(MultiAgentEnv):
    def __init__(
        ...
        self.replay_path = replay_path if replay_path is not None else os.path.join('render',\
                                                                                     self.map_name)
    ...
    def meta_step(self, actions):
        ...
        for plane, action in zip(self.planes, actions):
            ...
            if not plane.check_bounds():
                plane.alive = False
                plane.just_died = True
                plane.out_of_bound = True
                if plane.red:
                    self.n_red_alive -= 1
    ...
    def render(self, actions):
        ...
        if self.n_red_alive > self.n_blue_alive:
            who_is_winning = '红方领先'
        elif self.n_blue_alive < self.n_blue_alive:
            who_is_winning = '蓝方领先'
        else:
            who_is_winning = '双方持平'
